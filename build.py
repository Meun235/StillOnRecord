...
Reads data/fixture_dev.csv (swap SOURCE to the master file when the build is right),
validates the schema, and writes everything into site/.
...
SOURCE = os.path.join(ROOT, 'data', 'fixture_dev.csv')
REGISTER = os.path.join(ROOT, 'data', 'suffering_map_register.csv')
...
def shell(title, body, desc=TAGLINE, depth=0, body_class='', extra_head='', extra_js=''):
    ...
    nav = [('index.html', 'Map'), ('register.html', 'Register'), ('method.html', 'Method'),
           ('about.html', 'About')]
    links = ''.join(
        f'<a href="{up}{h}"{" aria-current=\"page\"" if h == title[1] else ""}>{t}</a>'
        for h, t in nav)
...
<br>Built from the development fixture: 62 records with unverified coordinates. Not for citation.
</div></footer>"""
...
<div class="wrap">
    ...
    <div class="idxtop">
      ...
      <div class="tline">
        ...
        <div class="track">
          ...
          <input type="range" id="r1" min="{YEAR_FLOOR}" max="2026" step="1"
            value="{YEAR_FLOOR}" aria-label="Start year">
          <input type="range" id="r2" min="{YEAR_FLOOR}" max="2026" step="1"
            value="2026" aria-label="End year">
        </div>
  ...
  <div class="mapwrap">
    ...
    <div class="key">
      ...
      Dashed ring: an area, not a point. Outer ring: more records underneath, zoom in.
      Unlocated records stay in the list and never get a pin.
    </div>
...
def build_record(d, recs):
    ...
    nearby = [x for x in recs if x['country_today'] == d['country_today']
              and x['id'] != d['id'] and x not in siblings and x not in children][:6]
    ...
    prose = ''
    if d.get('notes'):
        prose += f'<p class="lead">{e(d["notes"])}.</p>'
    if d.get('toll_note'):
...
<p class="subtitle"><span class="py">{e(years(d))}</span>, {e(d['place_name'] or '')}{
    ', ' + e(d['country_today']) if d['country_today'] and d['country_today'] != d['place_name'] else ''}</p>
<p class="statusline">{e(d['category'][:1].upper() + d['category'][1:])}. Tier {e(d['evidence_tier'])},
...
def build_register(recs):
  ...
  <p><span class="lab">Why it cannot be placed:</span> {e(d['register_reason'])}.</p>
  <p><span class="lab">What would place it:</span> {e(d['unlock'])}.</p>
</div>""" for d in reg)
...
<h2>Status</h2>
<p>Seed stage. This build carries {n} records from the development fixture, with coordinates
that are approximate and unverified. The full dataset holds 541 records, each naming a real and
relevant source, and none of those sources has yet been opened and checked one by one. Records
carry their state on their own page. Read it before citing anything here.</p>
...
def main():
    ...
    # A single-file copy of the map page with the css and js inlined. Useful for sending
    # someone a look at it without a server. The real site is the folder above.
    idx = pages['index.html']
    css = open(os.path.join(ROOT, 'assets', 'site.css'), encoding='utf-8').read()
    js = '\n'.join(open(os.path.join(ROOT, 'assets', f), encoding='utf-8').read()
                   for f in ('basemap.js', 'map.js'))
    idx = idx.replace('<link rel="stylesheet" href="assets/site.css">',
                      '<style>' + css + '</style>')
    idx = idx.replace('<script src="assets/basemap.js"></script>\n'
                      '<script src="assets/map.js"></script>',
                      '<script>' + js + '</script>')
    with open(os.path.join(OUT, 'preview-standalone.html'), 'w', encoding='utf-8') as f:
        f.write(idx)

    located = sum(1 for d in recs if d['lat'] is not None)
    print(f'{len(recs)} records, {located} located, {len(recs) - located} not plotted')
